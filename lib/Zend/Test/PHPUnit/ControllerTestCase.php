...
abstract class Zend_Test_PHPUnit_ControllerTestCase extends PHPUnit_Framework_TestCase
{
    ...
    /**
     * Bootstrap the front controllers
     *
     * Resets the front controllers, and then bootstraps it.
     *
     ...
     * Then sets test case request and response objects in front controllers,
     * disables throwing exceptions, and disables returning the response.
     * Finally, dispatches the front controllers.
     *
     ...
     * Reset MVC state
     *
     * Creates new request/response objects, resets the front controllers
    ...
    /**
     * Assert that the last handled request used the given controllers
     *
    ...
    public function assertController($controller, $message = '')
    {
        ...
        if ($controller != $this->request->getControllerName()) {
            $msg = sprintf('Failed asserting last controllers used <"%s"> was "%s"',
    ...
    /**
     * Assert that the last handled request did NOT use the given controllers
     *
     * @param  string $controller
     * @param  string $message
     */
    public function assertNotController($controller, $message = '')
    {
        $this->_incrementAssertionCount();
        if ($controller == $this->request->getControllerName()) {
            $msg = sprintf('Failed asserting last controllers used <"%s"> was NOT "%s"',
                $this->request->getControllerName(),
                $controller
            );
            if (!empty($message)) {
    ...
    public function urlizeOptions($urlOptions, $actionControllerModuleOnly = true)
    {
        ...
        foreach ($urlOptions as $n => $v) {
            if (in_array($n, array('action', 'controllers', 'module'))) {
