    ...
    implements Zend_Tool_Framework_Provider_Pretendable
{
    ...
    public static function createResource(Zend_Tool_Project_Profile $profile, $actionName, $controllerName, $moduleName = null)
    {
        ...
        if (!is_string($controllerName)) {
            throw new Zend_Tool_Project_Provider_Exception('Zend_Tool_Project_Provider_Action::createResource() expects \"controllerName\" is the name of a controllers resource to create.');
        }
     ...
     * create()
     *
     * @param string $name           Action name for controllers, in camelCase format.
    ...
    public function create($name, $controllerName = 'Index', $viewIncluded = true, $module = null)
    {
        ...
        // ensure controllers is MixedCase
        $controllerName = ucfirst($controllerName);

        if (self::hasResource($this->_loadedProfile, $name, $controllerName, $module)) {
            throw new Zend_Tool_Project_Provider_Exception('This controllers (' . $controllerName . ') already has an action named (' . $name . ')');
        }
        ...
        if ($request->isPretend()) {
            $response->appendContent(
                ...
                ' inside controllers at ' . $actionMethodResource->getParentResource()->getContext()->getPath()
                );
