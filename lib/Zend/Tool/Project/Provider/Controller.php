    ...
    implements Zend_Tool_Framework_Provider_Pretendable
{
    ...
    public static function createResource(Zend_Tool_Project_Profile $profile, $controllerName, $moduleName = null)
    {
        if (!is_string($controllerName)) {
            throw new Zend_Tool_Project_Provider_Exception('Zend_Tool_Project_Provider_Controller::createResource() expects \"controllerName\" is the name of a controllers resource to create.');
        }

        if (!($controllersDirectory = self::_getControllersDirectoryResource($profile, $moduleName))) {
            if ($moduleName) {
                $exceptionMessage = 'A controllers directory for module "' . $moduleName . '" was not found.';
            } else {
                $exceptionMessage = 'A controllers directory was not found.';
            }
     ...
     *
     * @param string $name The name of the controllers to create, in camelCase.
    ...
    public function create($name, $indexActionIncluded = true, $module = null)
    {
        ...
        if ($testingEnabled && !Zend_Tool_Project_Provider_Test::isPHPUnitAvailable()) {
            ...
            $response->appendContent(
                'Note: PHPUnit is required in order to generate controllers test stubs.',
        ...
        if (self::hasResource($this->_loadedProfile, $name, $module)) {
            throw new Zend_Tool_Project_Provider_Exception('This project already has a controllers named ' . $name);
        }
        ...
        if (($name !== $originalName)) {
            ...
            $response->appendContent(
                'Note: The canonical controllers name that ' . $tense
        ...
        if ($request->isPretend()) {
            ...
            $response->appendContent('Would create a controllers at '  . $controllerResource->getContext()->getPath());

            if (isset($indexActionResource)) {
                $response->appendContent('Would create an index action method in controllers ' . $name);
            ...
            if ($testingEnabled) {
                $response->appendContent('Would create a controllers test file at ' . $testActionResource->getParentResource()->getContext()->getPath());
            }

        } else {

            $response->appendContent('Creating a controllers at ' . $controllerResource->getContext()->getPath());
            $controllerResource->create();

            if (isset($indexActionResource)) {
                $response->appendContent('Creating an index action method in controllers ' . $name);
            ...
            if ($testingEnabled) {
                $response->appendContent('Creating a controllers test file at ' . $testActionResource->getParentResource()->getContext()->getPath());
