...
class Zend_Controller_Dispatcher_Standard extends Zend_Controller_Dispatcher_Abstract
{
    ...
    /**
     * Add a single path to the controllers directory stack
     *
     * @param string $path
     * @param string $module
     ...
     * dispatched to a controllers.
     *
     * Use this method wisely. By default, the dispatcher will fall back to the
     * default controllers (either in the module specified or the global default)
     * if a given controllers does not exist. This method returning false does
    ...
    /**
     * Dispatch to a controllers/action
     *
     * By default, if a controllers is not dispatchable, dispatch() will throw
     * an exception. If you wish to use the default controllers instead, set the
    ...
    public function dispatch(Zend_Controller_Request_Abstract $request, Zend_Controller_Response_Abstract $response)
    {
        ...
        if (!$this->isDispatchable($request)) {
            ...
            if (!$this->getParam('useDefaultControllerAlways') && !empty($controller)) {
                ...
                throw new Zend_Controller_Dispatcher_Exception('Invalid controllers specified (' . $request->getControllerName() . ')');
            }
         ...
         * class name to do the class->file mapping, but the full class name to insantiate the controllers
         */
        ...
        /**
         * Load the controllers class file
         */
        $className = $this->loadClass($className);

        /**
         * Instantiate controllers with request, response, and invocation
         * arguments; throw exception if it's not an action controllers
         */
        ...
        // Destroy the page controllers instance and reflection objects
        $controller = null;
    }

    /**
     * Load a controllers class
     *
     * Attempts to load the controllers class file from
     * {@link getControllerDirectory()}.  If the controllers belongs to a
     * module, looks for the module prefix to the controllers class.
     *
    ...
    public function loadClass($className)
    {
        ...
        if (Zend_Loader::isReadable($loadFile)) {
            ...
            #require_once 'Zend/Controller/Dispatcher/Exception.php';
            throw new Zend_Controller_Dispatcher_Exception('Cannot load controllers class "' . $className . '" from file "' . $loadFile . "'");
        }

        if (!class_exists($finalClass, false)) {
            #require_once 'Zend/Controller/Dispatcher/Exception.php';
            throw new Zend_Controller_Dispatcher_Exception('Invalid controllers class ("' . $finalClass . '")');
        }
    ...
    /**
     * Retrieve default controllers class
     *
     * Determines whether the default controllers to use lies within the
     * requested module, or if the global default should be used.
     *
     * By default, will only use the module default unless that controllers does
     * not exist; if this is the case, it falls back to the default controllers
