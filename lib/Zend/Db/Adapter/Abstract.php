...
public function insert($table, array $bind)
    ...
    public function insert_old($table, array $bind)
    {
try {
        // extract and quote col names from the array keys
        $cols = array();
...
/*        Mage::log(var_export($bind, true), null, 'importacao.log', true);

        if(isset($bind["product_id"])){
            $sqlSelect = "select * from cataloginventory_stock_item where product_id = " . $bind["product_id"];
            $retornoSelect = self::fetchAll($sqlSelect);

            if(count($retornoSelect) <= 0) {
...
Mage::log($e->getMessage(), null, 'importacao.log', true);
}    
}
