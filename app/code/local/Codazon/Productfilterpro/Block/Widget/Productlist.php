...
class Codazon_Productfilterpro_Block_Widget_Productlist  extends Mage_Catalog_Block_Product_Abstract implements Mage_Widget_Block_Interface
{
    ...
    {
        if ($this->getData('category_tabs')) {
            $template = 'codazon_productfilterpro/category-tabs.phtml';
        } elseif ($this->getData('use_ajax') == 1){
    ...
    public function getCategoryTabsItems()
    {
        $tabs = array();
        $children = false;
        $parent = Mage::getModel('catalog/category');
        if ($this->getData('use_selected_categories') && $this->getData('categories')) {
            $children = $this->getData('categories');
        } elseif ($parentId = $this->getData('parent_category')) {
            $parent->load($parentId);
            $children = $parent->getChildren();
        }
        if ($children) {
            $limit = $this->getData('max_cat_tabs_count'); 
            $collection = $parent->getResourceCollection();
            $collection->addFieldToFilter('entity_id', array('in' => explode(',', $children)))
            ...
            if ($collection->count()) {
                if ($this->getData('display_all_tab')) {
                    $tab = array();
                    $tab['title'] = $this->__('All');
                    $tab['content'] = $this->addData(array(
                        'categories'    => $children,
                        'category_tabs' => 0
                    ))->_toHtml();
                    $tabs[] = $tab;
                }
                $useAjax = $this->getData('use_ajax');
                foreach ($collection as $cat) {
                    $tab = array();
                    $tab['title'] = $cat->getName();
                    $tab['content'] = $this->addData(array(
