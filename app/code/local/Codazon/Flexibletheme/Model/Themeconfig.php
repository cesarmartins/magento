 ...
 */
class Codazon_Flexibletheme_Model_Themeconfig extends Mage_Core_Model_Config_Base
{   
    const CACHE_TAG         = 'THEME_CONFIG';
    ...
    {
        $this->setCacheId('theme_config_global');
    ...
    public function getCacheId() {
        return 'theme_config_global_' . $this->getThemeId();
    }
    ...
    public function getResourceModel()
    {
        if (is_null($this->_resourceModel)) {
            $this->_resourceModel = Mage::getResourceModel('flexibletheme/themeconfig');
        }
    ...
    public function loadModules()
    {
        ...
        $resourceConfig = sprintf('theme_config.%s.xml', $this->_getResourceConnectionModel('core'));
        $this->loadModulesConfiguration(array('theme_config.xml', $resourceConfig), $this);
        $this->loadThemeConfiguration($this);
        /**
    ...
    public function loadDb()
    {
        if ($this->_isLocalConfigLoaded && Mage::isInstalled()) {
            ...
            $condition = null;
            if (!is_null($this->getThemeId())) {
                $condition = '`theme_id` = \'' . $this->getThemeId() . '\'';
            }
            $dbConf->loadToXml($this, $condition);
    ...
    public function loadThemeConfiguration($mergeToObject = null, $mergeModel=null)
    {
        $themeId = $this->getThemeId();
        $theme = Mage::getModel('flexibletheme/theme')->load($themeId);
        if ($mergeModel === null) {
            $mergeModel = clone $this->_prototype;
        }
        if ($theme->getId()) {
            $configFile = Mage::getBaseDir('design') . DS . 'frontend' . DS . $theme->getData('theme_package') . DS . $theme->getData('theme_template') . DS .'etc'. DS . 'theme_config.xml';
    ...
    {
        $config = Mage::helper('flexibletheme')->getThemeConfig()->getNode($path);
        if (!$config) {
            return false;
        } else {
            $className = $config->getClassName();
     ...
     */
    public function saveConfig($themeId, $path, $value, $scope = 'default', $scopeId = 0)
    {
        $resource = $this->getResourceModel();
        $resource->saveConfig($themeId, rtrim($path, '/'), $value, $scope, $scopeId);
     ...
     * @return  Mage_Core_Model_Config
     */
    public function deleteConfig($themeId, $path, $scope = 'default', $scopeId = 0)
    {
        $resource = $this->getResourceModel();
        $resource->deleteConfig($themeId, rtrim($path, '/'), $scope, $scopeId);
