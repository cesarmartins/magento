...
class Codazon_Flexibletheme_Helper_Data extends Mage_Core_Helper_Abstract
{
    ...
    public function addScripts($layout)
    {
        ...
        if (!$this->isDeveloperMode()) {
            ...
            if ($this->getHeader()->cssFileExisted()) {
                $head->addItem('link_rel', $mediaUrl . $this->getHeader()->getMainCssFileRelativePath(), 'rel="stylesheet" type="text/css" media="all" ');
            }
            if ($this->getFooter()->cssFileExisted()) {
                $head->addItem('link_rel', $mediaUrl . $this->getFooter()->getMainCssFileRelativePath(), 'rel="stylesheet" type="text/css" media="all" ');
            }
            if ($this->getMainContent()->cssFileExisted()) {
                $head->addItem('link_rel', $mediaUrl . $this->getMainContent()->getMainCssFileRelativePath(), 'rel="stylesheet" type="text/css" media="all" ');
            }
        }
    }
	...
	public function getGoogleMapJavascriptUrl()
    {
		...
		return "//maps.googleapis.com/maps/api/js?v=3.31&key={$key}";
	}
