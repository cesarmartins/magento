...
class Seaway_Tree_Model_Tree
{
        ...
        try{
            ...
            if(is_numeric($parentId)){
               ...
               if(empty($limitFirst)){
                   ...
                   $search = " AND t.id in ($ids) ";
               }else{
                   $search = " AND  t.parent_id = $parentId ";
               }
            ...
            if(is_numeric($amigo)){
                if($amigo == 1){
                    $sqlAmigo = " AND t.instagram_status = '1' ";
                }elseif($amigo == 2){
                    $sqlAmigo = " AND t.instagram_status = '2' ";
                }elseif($amigo == 3){
                    $sqlAmigo = " AND t.instagram_status = '3' ";
                }
            }

            $this->niveis = array();

            /*$sqlParentType = " AND t.parent_id = 2 ";
            if($parentType != 'pai1'){
                $sqlParentType = " AND t.grandfather_id <> 1 ";
            }*/

            $sqlParentType = "";
            switch($parentType) {
                case 'pai':
                    $sqlParentType = " AND t.parent_id = 1 ";
                break;
                case 'pai1':
                    $sqlParentType = " AND t.grandfather_id = 1  ";
                break;
                case 'pai2':
                    $sqlParentType = " AND t.grandfather_id <> 1 AND t.grandfather_id <> 0 ";
                break;
                default:
                    $sqlParentType = " AND t.parent_id = 0 ";
                break;
            }
            

            $sqlSituation  =   " AND situation = 0 ";
            if($situation){
                $sqlSituation  = " AND situation = $situation ";
            }

            // filtra lista A por default
            $sqlStatus = " AND (status = 1 or status = 0) ";
            if($status == 2){
                // filtra lista B caso seja passado algum parametro
                $sqlStatus = " AND status = 2 ";
            }
            $ordersSql = "";
            $v = "";
            foreach($orders as  $order ){
                ...
                if($order['name'] == 't.parent_id') {
                    $order['name'] = 't.parent_name';
                }
                if($order['name'] == 't.last_action') {
                    $order['name'] =  't.last_action_date';
                }
                if($order['name'] == 't.future_action'){
                    $order['name'] = 't.future_action';
                }
                if($order['name'] == 't.last_exception'){
                    $order['name'] = 't.last_exception';
                }
            ...
            $sqlAction = "";
            if(!empty($lastAction)) {
                $sqlAction = " AND t.last_action_id = $lastAction  ";
            }

            $sqlDateDiffLastAction = "IF(DATEDIFF(DATE(NOW()),DATE(t.last_action_date)) >= 15 , 1 , 0)";
            $sqlNewListApp = "IF( t.new_list_app = 1 , 1 , 0)";
            $sqlNewListAppConfirm = "IF( t.new_list_app = 2 , 1 , 0)";
            $sqlNewListAppRefused = "IF( t.new_list_app = 3 , 1 , 0)";

            $sql = "SELECT t.id ,(0) as limite_ultrapassado,
                                 ($sqlDateDiffLastAction) as limite_ultrapassado_action,
                                 ($sqlNewListApp) as is_new_suggest,
                                 ($sqlNewListAppConfirm) as is_suggest_confirm,
                                 ($sqlNewListAppRefused) as is_suggest_refused,
                                 t.last_action ,
                                 t.future_action ,
                                 t.future_action_id ,
                                 IF((t.last_exception_des = '' or t.last_exception_des is null ) , t.last_exception , t.last_exception_des) as last_exception  ,
                                 t.parent_name as parent,
                                 t.parent_id ,
                                 IF(t.atleta = 1 ,CONCAT(t.nome , ' - ', 'atleta') , t.nome ) as child ,
                                 t.estado ,
                                 t.telefone ,
                                 t.status ,
                                 t.old_process ,
            ...
            FROM t_tree t  WHERE  1 = 1 $sqlStatus $sqlSituation $sqlAction $sqlParentType $sqlAmigo $search  $ordersSql limit $offset,$limit ";


            
            $sqlTotal = "SELECT CEILING(count(*)/$limit) as total_pag ,count(*) as total FROM t_tree t WHERE 1 = 1 $sqlStatus $sqlSituation $sqlAction $sqlParentType $sqlAmigo $search $ordersSql ";
            ...
            $totais =  $this->getSituationQty($sqlStatus , $sqlAction ,$sqlParentType);
    ...
    public function updateTreeAction($treeId , $actionId){
   ...
   public function isParticipantByCustomerId($customerId , $email = false){
    ...
    public  function enableBring(){
        try{
            ...
            if(!empty($coupon) && !empty($coupon['is_special'])){
                $data = array();
