...
class Mage_Core_Controller_Front_Router
{
    ...
    public function addRoutes(Zend_Controller_Router_Interface $router)
    {
        ...
        $routeMatch = $frontName.'/:controllers/:action/*';
        $moduleName = (string)$this->_config->module;
        $routeParams = array('module'=>$moduleName, 'controllers'=>'index', 'action'=>'index', '_frontName'=>$frontName);
    ...
    {
        static $reservedKeys = array('module'=>1, 'controllers'=>1, 'action'=>1, 'array'=>1);
        
        if (is_string($params)) {
            $paramsArr = explode('/', $params);
            $params = array('controllers'=>$paramsArr[0], 'action'=>$paramsArr[1]);
        }
        ...
        if (!empty($params)) {
            ...
            if (empty($params['controllers']) && !empty($paramsStr)) {
                $params['controllers'] = 'index';
            }
            $url .= empty($params['controllers']) ? '' : $params['controllers'].'/';
