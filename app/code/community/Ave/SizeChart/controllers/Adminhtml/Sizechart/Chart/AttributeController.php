...
/**
 * Chart admin attribute controllers
 *
 * @category    Ave
 * @package     Ave_SizeChart
 * @author      averun <dev@averun.com>
 */
class Ave_SizeChart_Adminhtml_SizeChart_Chart_AttributeController extends Mage_Adminhtml_Controller_Action
{
    ...
    public function preDispatch()
    {
        ...
        $this->_entityTypeId = Mage::getModel('eav/entity')
            ->setType(Ave_SizeChart_Model_Chart::ENTITY)
            ->getTypeId();
    }
     ...
     *
     * @accees protected
     * @return Ave_SizeChart_Adminhtml_Chart_AttributeController
     * @author averun <dev@averun.com>
     */
    protected function _initAction()
    {
        $this->_title(Mage::helper('ave_sizechart')->__('Chart'))
             ->_title(Mage::helper('ave_sizechart')->__('Attributes'))
             ->_title(Mage::helper('ave_sizechart')->__('Manage Attributes'));

        $this->loadLayout()
            ->_setActiveMenu('catalog/ave_sizechart/chart_attributes')
            ->_addBreadcrumb(
                Mage::helper('ave_sizechart')->__('Chart'),
                Mage::helper('ave_sizechart')->__('Chart')
            )
            ->_addBreadcrumb(
                Mage::helper('ave_sizechart')->__('Manage Chart Attributes'),
                Mage::helper('ave_sizechart')->__('Manage Chart Attributes')
            );
        return $this;
    }
    ...
    public function editAction()
    {
        ...
        if ($id) {
            ...
            if (! $model->getId()) {
                Mage::getSingleton('adminhtml/session')->addError(
                    Mage::helper('ave_sizechart')->__('This chart attribute no longer exists')
                );
                $this->_redirect('*/*/');
                return;
            }
            ...
            if ($model->getEntityTypeId() != $this->_entityTypeId) {
                Mage::getSingleton('adminhtml/session')->addError(
                    Mage::helper('ave_sizechart')->__('This chart attribute cannot be edited.')
                );
                $this->_redirect('*/*/');
                return;
            }
        ...
        $this->_initAction();
        $this->_title($id ? $model->getName() : Mage::helper('ave_sizechart')->__('New Chart Attribute'));
        $item = $id ? Mage::helper('ave_sizechart')->__('Edit Chart Attribute')
                    : Mage::helper('ave_sizechart')->__('New Chart Attribute');
    ...
    public function saveAction()
    {
        ...
        if ($data) {
            ...
            $model        = Mage::getModel('ave_sizechart/resource_eav_attribute');
            $helper       = Mage::helper('ave_sizechart/chart');
            $id           = $this->getRequest()->getParam('attribute_id');
            ...
            try {
                $model->save();
                $session->addSuccess(
                    Mage::helper('ave_sizechart')->__('The chart attribute has been saved.')
                );
                /**
        ...
        if ($id = $this->getRequest()->getParam('attribute_id')) {
            ...
            try {
                ...
                Mage::getSingleton('adminhtml/session')->addSuccess(
                    Mage::helper('ave_sizechart')->__('The chart attribute has been deleted.')
                );
                $this->_redirect('*/*/');
                return;
            }
    ...
    {
        return Mage::getSingleton('admin/session')->isAllowed('catalog/ave_sizechart/chart_attributes');
    }
