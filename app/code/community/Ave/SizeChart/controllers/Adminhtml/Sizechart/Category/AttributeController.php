...
/**
 * Category of sizes admin attribute controllers
 *
 * @category    Ave
 * @package     Ave_SizeChart
 * @author      averun <dev@averun.com>
 */
class Ave_SizeChart_Adminhtml_SizeChart_Category_AttributeController extends Mage_Adminhtml_Controller_Action
{
    ...
    public function preDispatch()
    {
        ...
        $this->_entityTypeId = Mage::getModel('eav/entity')
            ->setType(Ave_SizeChart_Model_Category::ENTITY)
            ->getTypeId();
    }
     ...
     *
     * @accees protected
     * @return Ave_SizeChart_Adminhtml_Category_AttributeController
     * @author averun <dev@averun.com>
     */
    protected function _initAction()
    {
        $this->_title(Mage::helper('ave_sizechart')->__('Category of sizes'))
             ->_title(Mage::helper('ave_sizechart')->__('Attributes'))
             ->_title(Mage::helper('ave_sizechart')->__('Manage Attributes'));

        $this->loadLayout()
            ->_setActiveMenu('catalog/ave_sizechart/category_attributes')
            ->_addBreadcrumb(
                Mage::helper('ave_sizechart')->__('Category of sizes'),
                Mage::helper('ave_sizechart')->__('Category of sizes')
            )
            ->_addBreadcrumb(
                Mage::helper('ave_sizechart')->__('Manage Category of sizes Attributes'),
                Mage::helper('ave_sizechart')->__('Manage Category of sizes Attributes')
            );
        return $this;
    }
    ...
    public function editAction()
    {
        ...
        if ($id) {
            ...
            if (! $model->getId()) {
                Mage::getSingleton('adminhtml/session')->addError(
                    Mage::helper('ave_sizechart')->__('This category of sizes attribute no longer exists')
                );
                $this->_redirect('*/*/');
                return;
            }
            ...
            if ($model->getEntityTypeId() != $this->_entityTypeId) {
                Mage::getSingleton('adminhtml/session')->addError(
                    Mage::helper('ave_sizechart')->__('This category of sizes attribute cannot be edited.')
                );
                $this->_redirect('*/*/');
                return;
            }
        ...
        $this->_initAction();
        $this->_title($id ? $model->getName() : Mage::helper('ave_sizechart')->__('New Category of sizes Attribute'));
        $item = $id ? Mage::helper('ave_sizechart')->__('Edit Category of sizes Attribute')
                    : Mage::helper('ave_sizechart')->__('New Category of sizes Attribute');
    ...
    public function saveAction()
    {
        ...
        if ($data) {
            ...
            $model        = Mage::getModel('ave_sizechart/resource_eav_attribute');
            $helper       = Mage::helper('ave_sizechart/category');
            $id           = $this->getRequest()->getParam('attribute_id');
            ...
            try {
                $model->save();
                $session->addSuccess(
                    Mage::helper('ave_sizechart')->__('The category of sizes attribute has been saved.')
                );
                /**
        ...
        if ($id = $this->getRequest()->getParam('attribute_id')) {
            ...
            try {
                ...
                Mage::getSingleton('adminhtml/session')->addSuccess(
                    Mage::helper('ave_sizechart')->__('The category of sizes attribute has been deleted.')
                );
                $this->_redirect('*/*/');
                return;
            }
    ...
    {
        return Mage::getSingleton('admin/session')->isAllowed('catalog/ave_sizechart/category_attributes');
    }
     ...
     * @param $attributeCode
     */
    protected function validateAttributeCode($session, $id, $attributeCode)
    {
            ...
            if (!$validatorAttrCode->isValid($attributeCode)) {
                $session->addError(
                    Mage::helper('ave_sizechart')->__(
                        'Attribute code is invalid. Please use only letters (a-z), numbers (0-9) or underscore(_) in '
