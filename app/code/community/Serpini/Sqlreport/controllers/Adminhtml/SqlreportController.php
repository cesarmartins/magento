    	...
    	if($action!=""){
    		switch ($action) {
    			...
    			case "testConnection" : $db_host=$this->getRequest()->getParam('db_host');
    				...
    				break;
    			case "executeCron"	:	$reportCode=$this->getRequest()->getParam('code');
    									$cronObserver = Mage::getModel('sqlreport/CronObserver');
    									$res= $cronObserver->executeCronReport($reportCode);
    									if(true===$res){
    										$data[0] = array("type" => "success-msg",
    														"msg" => Mage::helper('catalog')->__("Executed report like scheduled correctly"), 
    														"status" => "ok",
    														"logExecutions" => $cronObserver->getLogExecutions()
    										);
    									}else{
    										$data[0] = array("type" => "error-msg","msg" => $res, "status" => "ko");
    									}
    									$result= json_encode($data);
    				break;
