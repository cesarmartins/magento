...
class Serpini_Sqlreport_Model_Report extends Mage_Core_Model_Abstract
{
	...
	protected $tickChartLabel = array();
	
	protected $logger;
	
	protected $atributesDefault = array(
			"pdfDefault" => array("pdfLogoUrl","pdfLogoWidth","pdfLogoHeight","pdfLogoMarginTop","pdfLogoMarginLeft",
						"pdfTitleFontName","pdfTitleFontSize","pdfTitleFontBold","pdfTitleFontItalic","pdfTitleFontColor","pdfTitleMarginTop","pdfTitleMarginLeft",
						"pdfDescriptionFontName","pdfDescriptionFontSize","pdfDescriptionAlignLeft","pdfDescriptionAlignCenter","pdfDescriptionAlignRight","pdfDescriptionFontBold","pdfDescriptionFontItalic","pdfDescriptionFontColor","pdfDescriptionSpace","pdfDescriptionMarginTop","pdfDescriptionMarginLeft",
						"pdfFiltersShow	","pdfFiltersFontName","pdfFiltersFontSize","pdfFilterFontBold","pdfFilterFontItalic","pdfFilterFontColor","pdfFiltersMarginLeft","pdfFiltersMarginTop",
						...
						"pdfTableMarginTop",
						"pdfFooterFontName","pdfFooterFontSize","pdfFooterAlignLeft","pdfFooterAlignCenter","pdfFooterAlignRight","pdfFooterFontBold","pdfFooterFontItalic","pdfFooterFontColor","pdfFooterString","pdfFooterMarginTop","pdfFooterMarginBottom",
						"pdfPageMarginTop","pdfPageMarginLeft","pdfPageMarginBottom","pdfPageMarginRight","pdfPageSize","pdfPageOrientation" ),
	...
	// Si uno de éstos está seteado como true, los otros se pone a false
    ...
    protected function _construct(){	
        ...
        $this->fontPath = $importPath = Mage::getBaseDir().DS."app".DS."code".DS."community".DS."Serpini".DS."Sqlreport".DS."Model".DS."PdfTable".DS."fonts";
    }
    ...
    // Elimina un combo por su parámetro
   ...
   /*  public function getXlsAtribute($atribute){
    ...
    private function loadAtributesDefault(){
    	...
    	foreach ($readresult as $fila){
    		...
    		if(!array_key_exists($fila['type_code'],$this->atributes)){
    			...
    			// El valor está setado, comprobamos si el de por defecto (check) indica que se ha de coger el seteado o el por defecto
    ...
    // Añade con valor false a la lista de atributos, los atributos agrupados únicos
    ...
    public function printJSChart(){
    	...
    	switch ($this->getAtribute('chart_type')){
    		case 'lineChart': 
    			...
    			if(count($this->tickChartLabel)>0){
    				$salida .= 'var labels = [';
      				foreach ($this->tickChartLabel as $label){
      					$salida .= "".$label.",";
      				}
      				
      				$salida = substr($salida,0,strlen ($salida)-1);
      				$salida .="];";
      				$salida .="chart.xAxis.tickValues([";
      				for($i=0;$i<count($this->tickChartLabel);$i++){
      					$salida.=$i.",";
      				}
      				$salida = substr($salida,0,strlen ($salida)-1);
      				$salida .="]).tickFormat(function(d){
      					return labels[d]
      				}).rotateLabels(-45);";
      				$salida .= " chart.margin({bottom: 100});";
    			}
    			break;
    ...
    public function printDataChart(){
    	if(""==$this->getAtribute('chart_seriefix') || "1"==$this->getAtribute('chart_seriefix')){
    		return $this->printDataChartFixedSerie();
    	}else{
    		return $this->printDataChartFixedX();
    	}
    	
    }
    
    private function getTickValue($label){
    	if(!in_array($label, $this->tickChartLabel)){
    		array_push($this->tickChartLabel, $label);
    	}
    	return array_search($label,$this->tickChartLabel);
    }
    
    private function printDataChartFixedSerie(){
    	$xName = $this->getColumnsName();
    	$xName = $xName[$this->getAtribute('chartXValue') - 1];
    	$salida = "data = [";
    		...
    		switch ($this->getAtribute('chart_type')){
    			...
    			case 'multiBarChart':
    				...
    				foreach ($this->getResults() as $result){
    					$valuePrs = $this->prs($result[$yName]);
    					$valueX = $this->prs($result[$xName]);
    					if(!is_numeric($valueX)) $valueX=$this->getTickValue($valueX);
    					if(""!=$valuePrs) $salida .= "{x:".$valueX.",y:".$this->prs($result[$yName])."},";
    				}
    				$salida .= "]},";
    				break;
    			case 'stackedAreaChart':
    				...
    				foreach ($this->getResults() as $result){
    					$valuePrs = $this->prs($result[$yName]);
    					if(""!=$valuePrs) $salida .= "[".$this->prs($result[$xName]).",".$this->prs($result[$yName])."],";
    				}
    				$salida .= "]},";
    				break;
    			case 'discreteBarChart':
    			case 'multiBarHorizontalChart':
    				...
    				foreach ($this->getResults() as $result){
    					$valuePrs = $this->prs($result[$yName]);
    					if(""!=$valuePrs) $salida .= "{\"label\":".$this->prs($result[$xName]).",\"value\":".$this->prs($result[$yName])."},";
    				}
    				$salida .= "]},";
    				break;
    			case 'lineAndBar':
    ...
    private function printDataChartFixedX(){
    	$columnsName = $this->getColumnsName();
    	$xName = $columnsName[$this->getAtribute('chartXValue') - 1];
    	$salida = "data = [";
    	foreach ($this->getResults() as $result){
    		switch ($this->getAtribute('chart_type')){
    			case 'lineChart':
    			case 'multiBarChart':
    				$salida .="{key: \"".$result[$xName]."\", values: [ ";
    				for($i = 1; $i<= sizeof($this->chart_series);$i++){
    					$columnNumber = $this->chart_series[$i];
    					$valuePrs = $this->prs($result[$columnsName[$columnNumber - 1]]);
    					$valueX = $this->prs($columnsName[$columnNumber - 1]);
    					if(!is_numeric($valueX)) $valueX=$this->getTickValue($valueX);
    					if(""!=$valuePrs) $salida .= "{x:".$valueX.",y:".$valuePrs."},";
    				}
    				$salida .= "]},";
    				break;
    			case 'stackedAreaChart':
    				...
    				for($i = 1; $i<= sizeof($this->chart_series);$i++){
    					$columnNumber = $this->chart_series[$i];
    					$valuePrs = $this->prs($result[$columnsName[$columnNumber - 1]]);
    					if(""!=$valuePrs) $salida .= "[".$this->prs($columnsName[$columnNumber - 1]).",".$valuePrs."],";
    				}
    				$salida .= "]},";
    				break;
    			case 'discreteBarChart':
    			case 'multiBarHorizontalChart':
    				...
    				for($i = 1; $i<= sizeof($this->chart_series);$i++){
    					$columnNumber = $this->chart_series[$i];
    					$valuePrs = $this->prs($result[$columnsName[$columnNumber - 1]]);
    					if(""!=$valuePrs) $salida .= "{\"label\":".$this->prs($columnsName[$columnNumber - 1]).",\"value\":".$valuePrs."},";
    				}
    				$salida .= "]},";
    				break;
    			case 'lineAndBar':
    				$salida .="{key: \"".$result[$xName]."\",bar: true, values: [ ";
    				for($i = 1; $i<= sizeof($this->chart_series);$i++){
    					$columnNumber = $this->chart_series[$i];
    					$valuePrs = $this->prs($result[$columnsName[$columnNumber - 1]]);
    					if(""!=$valuePrs) $salida .= "{\"label\":".$this->prs($columnsName[$columnNumber - 1]).",\"value\":".$valuePrs."},";
    ...
    private function prs($value){
    	$valueNumeric = str_replace(",",".",$value);
    	if(is_numeric($valueNumeric)) {
    		return $valueNumeric==0?"":$valueNumeric;
    	}else if(""==$value){
    		return "";
    	}else{
    		return '"'.$value.'"';
    ...
    public function exportPDFResult($fileName,$path,$resultSet,$offset=1){
    	try {
    		...
    		// Page Style
    		$pdf->setMargin(Serpini_Sqlreport_Model_PdfTable_Pdf::TOP,$this->getAtribute("pdfPageMarginTop"));
    		$pdf->setMargin(Serpini_Sqlreport_Model_PdfTable_Pdf::RIGHT,$this->getAtribute("pdfPageMarginRight"));
    		$pdf->setMargin(Serpini_Sqlreport_Model_PdfTable_Pdf::BOTTOM,$this->getAtribute("pdfPageMarginBottom"));
    		$pdf->setMargin(Serpini_Sqlreport_Model_PdfTable_Pdf::LEFT,$this->getAtribute("pdfPageMarginLeft"));
    		
    		// header
    		$header = $this->getPDFHeader();
    		if(false!=$header){
    			$pdf->setHeader($header);
    			$pdf->setDescriptionMargin(Serpini_Sqlreport_Model_PdfTable_Pdf::TOP,$this->getAtribute("pdfDescriptionMarginTop"));
    			$pdf->setDescriptionMargin(Serpini_Sqlreport_Model_PdfTable_Pdf::LEFT,$this->getAtribute("pdfDescriptionMarginLeft"));
    		}
    		// insert logo
    		...
    		$pdf->setTitleMargin(Serpini_Sqlreport_Model_PdfTable_Pdf::TOP, $this->getAtribute("pdfTitleMarginTop"));
    		$pdf->setTitleMargin(Serpini_Sqlreport_Model_PdfTable_Pdf::LEFT, $this->getAtribute("pdfTitleMarginLeft"));
    		
    		// insert footer
    		$footer = $this->getPDFFooter();
    		if($footer instanceof Serpini_Sqlreport_Model_PdfTable_Table){
    			$pdf->setFooter($footer);
    			$pdf->setFooterMargin(Serpini_Sqlreport_Model_PdfTable_Pdf::TOP, $this->getAtribute("pdfFooterMarginTop"));
    			$pdf->setFooterMargin(Serpini_Sqlreport_Model_PdfTable_Pdf::BOTTOM, $this->getAtribute("pdfFooterMarginBottom"));
    		}
    		
    		// create page
    		$page = $pdf->createPage($this->getPdfPageSize($this->getAtribute("pdfPageSize"),$this->getAtribute("pdfPageOrientation")));
    		...
    		// add table to page
    		if($table instanceof Serpini_Sqlreport_Model_PdfTable_Table){
    			$page->setResults($table, 0, $this->getAtribute("pdfDescriptionMarginTop"));
    		}
    ...
    private function getPDFTable($resultSet,$offset){
    	...
    	$table->setMargin(Serpini_Sqlreport_Model_PdfTable_Pdf::TOP, $this->getAtribute("pdfTableMarginTop"));
    	$table->setMargin(Serpini_Sqlreport_Model_PdfTable_Pdf::LEFT, $this->getAtribute("pdfTableMarginLeft"));
    	$table->setMargin(Serpini_Sqlreport_Model_PdfTable_Pdf::RIGHT, $this->getAtribute("pdfTableMarginRight"));
    	...
    	if("true"==$this->getAtribute('pdfFiltersShow')){
	    	...
	    	foreach ($this->getComboList() as $combo){
	    		if($combo->getType()!="evaluated" && ""!=$combo->getTitle()){
	    			...
	    			if($primero){
	    				$row->setCellPaddings(array(2+$this->getAtribute("pdfFiltersMarginTop"),2,2,$this->getAtribute("pdfFiltersMarginLeft")));
	    			}else{
	    				$row->setCellPaddings(array(2,2,2,$this->getAtribute("pdfFiltersMarginLeft")));
	    			}
    ...
    private function pdfInsertLogo($document){
    	...
    	if (file_exists($fullFileName)) {
    		$fileInfo = new finfo();
    		$typeFile=$fileInfo->file($fullFileName, FILEINFO_MIME_TYPE);
    		//$typeFile=mime_content_type($fullFileName);
    		...
    		$document->setLogo($image,$this->getAtribute("pdfLogoWidth"),$this->getAtribute("pdfLogoHeight"),$this->getAtribute("pdfLogoMarginTop"),$this->getAtribute("pdfLogoMarginLeft"));
    	}
    ...
    private function getPdfFontName($name,$bold,$italic){
    	...
    	switch ($name){
    		case "Courier": if("false"==$bold && "false"==$italic){
    							$nameFont = Zend_Pdf_Font::FONT_COURIER;
    						}else if("true"==$bold && "false"==$italic){
    							$nameFont = Zend_Pdf_Font::FONT_COURIER_BOLD;
    						}else if("false"==$bold && "true"==$italic){
    							$nameFont = Zend_Pdf_Font::FONT_COURIER_ITALIC;
    						}else if("true"==$bold && "true"==$italic){
    							$nameFont = Zend_Pdf_Font::FONT_COURIER_BOLD_ITALIC;
    						}
    						$font = Zend_Pdf_Font::fontWithName($nameFont);
    			break;
    		case "Helvetica":if("false"==$bold && "false"==$italic){
    							$nameFont = Zend_Pdf_Font::FONT_HELVETICA;
    						}else if("true"==$bold && "false"==$italic){
    							$nameFont = Zend_Pdf_Font::FONT_HELVETICA_BOLD;
    						}else if("false"==$bold && "true"==$italic){
    							$nameFont = Zend_Pdf_Font::FONT_HELVETICA_ITALIC;
    						}else if("true"==$bold && "true"==$italic){
    							$nameFont = Zend_Pdf_Font::FONT_HELVETICA_BOLD_ITALIC;
    						}
    						$font = Zend_Pdf_Font::fontWithName($nameFont);
    			break;
    		case "Times":if("false"==$bold && "false"==$italic){
    							$nameFont = Zend_Pdf_Font::FONT_TIMES;
    						}else if("true"==$bold && "false"==$italic){
    							$nameFont = Zend_Pdf_Font::FONT_TIMES_BOLD;
    						}else if("false"==$bold && "true"==$italic){
    							$nameFont = Zend_Pdf_Font::FONT_TIMES_ITALIC;
    						}else if("true"==$bold && "true"==$italic){
    							$nameFont = Zend_Pdf_Font::FONT_TIMES_BOLD_ITALIC;
    						}
    						$font = Zend_Pdf_Font::fontWithName($nameFont);
    			break;
    		default:		if("false"==$bold && "false"==$italic){
    							$nameFont = $name.".ttf";
    						}else if("true"==$bold && "false"==$italic){
    							$nameFont = $name."b.ttf";
    						}else if("false"==$bold && "true"==$italic){
    							...
    							$nameFont = $name."bi.ttf";
    						}
    						$font = Zend_Pdf_Font::fontWithPath($this->fontPath.DS.$nameFont);
    ...
    private function getPdfPageSize($size,$orientation){
    	$output = Zend_Pdf_Page::SIZE_A4; // Default value
    	
    	switch ($size){
    		case "LETTER": $output = ("Portrait"==$orientation?Zend_Pdf_Page::SIZE_LETTER:Zend_Pdf_Page::SIZE_LETTER_LANDSCAPE);
    			break;
    		case "A4": $output = ("Portrait"==$orientation?Zend_Pdf_Page::SIZE_A4:Zend_Pdf_Page::SIZE_A4_LANDSCAPE);
    			break;
    		case "A3": $output = ("Portrait"==$orientation?Serpini_Sqlreport_Model_PdfTable_Pdf::SIZE_A3:Serpini_Sqlreport_Model_PdfTable_Pdf::SIZE_A3_LANDSCAPE);
    			break;
    		case "A5": $output = ("Portrait"==$orientation?Serpini_Sqlreport_Model_PdfTable_Pdf::SIZE_A5:Serpini_Sqlreport_Model_PdfTable_Pdf::SIZE_A5_LANDSCAPE);
    			break;
