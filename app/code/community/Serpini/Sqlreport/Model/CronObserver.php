...
class Serpini_Sqlreport_Model_CronObserver {
	...
	private $_logExecutions = array();
	
	public function getLogExecutions(){
		return $this->_logExecutions;
	}
	...
	public function executeReport($schedule){
		...
		$this->executeCronReport($report_id);
		 
	}
    
	public function executeCronReport($report_id){
    	...
    	$cronNoRow = (""==$report->getAtribute('cronNoRow')?"false":$report->getAtribute('cronNoRow'));
    	Mage::log(get_class($this).'.'.__FUNCTION__.' numero de filas:'.$report->getSize()." cronNoRow:".$cronNoRow, Zend_Log::INFO,'sqlreport.log');
    	//if(!($report->getSize()==0 && (""==$report->getAtribute('cronNoRow')||"false"==$report->getAtribute('cronNoRow')))){
    	if($report->getSize()>0||($report->getSize()==0 && "false"==$cronNoRow)){
    ...
    private function prepareEmail($offset,$report,$resultSet){
    	...
    	if($report->isLoaded()){
    		$result="";
    		$fileName='sql2report_'.strtolower($report->getId())."_".date("YmdHi").rand(0,100);
    		if(""!=$report->getAtribute("cronFileName")) $fileName=$report->setParamsTitle($report->getAtribute("cronFileName"));
    		$filePath="";
    		$fileType="";
	    	...
	    	$error = $utils->sendMail($text,$to,$subject,$cc,$bcc,$fileName,$filePath,$fileType);
	    	 
	    	$logger->logReportEmail($report,$text,$to,$subject,$cc,$bcc,$error);
	    	array_push($this->_logExecutions,array('to'=>$to,
	    										   'cc'=>$cc,
	    										   'bcc'=>$bcc,
	    										   'subject'=>$subject,
	    										   'error'=>$error));
