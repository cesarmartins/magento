...
class Serpini_Sqlreport_Model_Combo extends Mage_Core_Model_Abstract
{
	...
	public function printHTML($haspermissionEdit = true){
		$result = "";
		switch ($this->type) {
			case "select": if("checkbox-unique"==$this->getAtribute("selectType")) {
								...
								$result = $this->printHTMLCheck($haspermissionEdit);
							} else {
								
								$result = $this->printHTMLSelect($haspermissionEdit);
							}
						
				break;
			case "date": $result = $this->printHTMLDate($haspermissionEdit);
				break;
			case "text": $result = $this->printHTMLText($haspermissionEdit);
				break;
			case "set": $result = $this->printHTMLSet($haspermissionEdit);
				break;
			default:
				;
			break;
	...
	private function printHTMLSelect($haspermissionEdit){
		$class = "select";
		...
		$result .= "</select> ";
		if($haspermissionEdit||$this->setupmanager->getValue('showSQLReadOnly')=='true'){
	...
	public function delete(){
		...
		try{
			...
			// Comprobamos que no haya ningún informe asociado a dicho combo
			$select = $connection->select()
