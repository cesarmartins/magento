...
class Serpini_Sqlreport_Model_PdfTable_Table {
	...
	private function getLogoTitleHeight($page){
		$logo=0;
		if($page->hasLogo()){
			$logo = $page->getLogoHeight();
		}
		
		$titulo=$page->getTitleHeight();
				
		return max($logo,$titulo);
	}
	...
	public function render(Serpini_Sqlreport_Model_PdfTable_Page $page, $posX,$posY,$inContentArea=true){
		...
		if($inContentArea){
			//echo "posY:".$posY;
			$start_y=$posY-$this->_margin[Serpini_Sqlreport_Model_PdfTable_Pdf::TOP];
			$max_y=$page->getHeight()- $page->getMargin(Serpini_Sqlreport_Model_PdfTable_Pdf::BOTTOM)-$page->getMargin(Serpini_Sqlreport_Model_PdfTable_Pdf::TOP)-$page->getFooterHeight()-15;
			
			if($page->hasHeader()){
				$start_y = $start_y+$page->getHeaderHeight($page);
			}
			$posX += $this->_margin[Serpini_Sqlreport_Model_PdfTable_Pdf::LEFT];
		}else{
			$start_y=  $posY;
		...
		foreach($this->_rows as $row) {
			...
			if($test>$max_y || $row->hasPageBreak()){
				...
				$nPage->setLogo($page->getLogo());
				$nPage->setDocument($page->getDocument());
				
				$page=$nPage;
				$this->_pages[] = $page;
				$y=$this->getLogoTitleHeight($page)+$this->_margin[Serpini_Sqlreport_Model_PdfTable_Pdf::TOP];
	...
	private function _preRender(Serpini_Sqlreport_Model_PdfTable_Page $page, $posX,$posY,$inContentArea=true){
		...
		foreach($this->_rows as $row) {
			...
			//pre-render row
			$row->setMargin(Serpini_Sqlreport_Model_PdfTable_Pdf::LEFT, $this->_margin[Serpini_Sqlreport_Model_PdfTable_Pdf::LEFT]);
			$row->setMargin(Serpini_Sqlreport_Model_PdfTable_Pdf::RIGHT, $this->_margin[Serpini_Sqlreport_Model_PdfTable_Pdf::RIGHT]);
