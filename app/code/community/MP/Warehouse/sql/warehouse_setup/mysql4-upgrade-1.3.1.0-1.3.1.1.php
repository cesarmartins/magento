<?php
/**
 * Mage Plugins, Co.
 *
 * NOTICE OF LICENSE
 *
 * This source file is subject to the Mage Plugins Commercial License (MPCL 1.0)
 * that is bundled with this package in the file LICENSE.md.
 * It is also available through the world-wide-web at this URL:
 * https://mageplugins.net/commercial-license/
 * If you did not receive a copy of the license and are unable to
 * obtain it through the world-wide-web, please send an email
 * to mageplugins@gmail.com so we can send you a copy immediately.
 *
 * DISCLAIMER
 *
 * Do not edit or add to this file if you wish to upgrade to newer
 * versions in the future. If you wish to customize the extension for your
 * needs please refer to http://www.mageplugins.net for more information.
 *
 * @category   MP
 * @package    MP_Warehouse
 * @copyright  Copyright (c) 2017-2018 Mage Plugins, Co. and affiliates (https://mageplugins.net/)
 * @license    https://mageplugins.net/commercial-license/ Mage Plugins Commercial License (MPCL 1.0)
 */

/* @var $installer Mage_Core_Model_Resource_Setup */
$installer = $this;

/** @var Varien_Db_Adapter_Pdo_Mysql $connection */
$connection = $installer->getConnection();

$installer->startSetup();

/**
 * Catalog Inventory Stock Item
 *
 * @see Mage_CatalogInventory_Model_Stock_Item
 */
$connection->update(
    $installer->getTable('cataloginventory/stock_item'),
    array('is_in_stock' => 0),
    array('is_in_stock = ?' => 1, 'qty = ?' => 0)
);

/**
 * Catalog Inventory Stock Status
 *
 * @see Mage_CatalogInventory_Model_Stock_Status
 */
$connection->update(
    $installer->getTable('cataloginventory/stock_status'),
    array('stock_status' => 0),
    array('stock_status = ?' => 1, 'qty = ?' => 0)
);

/**
 * Catalog Inventory Stock Status Indexer
 *
 * @see Mage_CatalogInventory_Model_Resource_Indexer_Stock
 */
$connection->update(
    $installer->getTable('cataloginventory/stock_status_indexer_idx'),
    array('stock_status' => 0),
    array('stock_status = ?' => 1, 'qty = ?' => 0)
);

$installer->endSetup();
