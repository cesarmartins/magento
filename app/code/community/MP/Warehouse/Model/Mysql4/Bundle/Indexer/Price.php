 ...
 */
class MP_Warehouse_Model_Mysql4_Bundle_Indexer_Price 
    extends Mage_Bundle_Model_Mysql4_Indexer_Price
{
    ...
    protected function _calculateBundleSelectionPrice($priceType)
    {
        ...
        if ($this->getVersionHelper()->isGe1600()) {
            ...
            if ($priceType == Mage_Bundle_Model_Product_Price::PRICE_TYPE_FIXED) {
                ...
                $priceExpr = new Zend_Db_Expr(
                    "IF(i.special_price > 0, ROUND(idx.min_price * (i.special_price / 100), 4), "
                    . "idx.min_price) * bs.selection_qty"
                );
                $tierExpr = new Zend_Db_Expr(
                    "IF(i.base_tier IS NOT NULL, ROUND(idx.min_price * (i.base_tier / 100), 4) "
        ...
        $select = $write->select()
            ...
            ->join(
                array('idx' => $this->getIdxTable()), '(bs.product_id = idx.entity_id) AND (i.customer_group_id = idx.customer_group_id) AND '.
    ...
    protected function _prepareBundlePrice($entityIds = null)
    {
        ...
        Mage::dispatchEvent(
            'prepare_catalog_product_price_index_table', array(
