 ...
 */
class MP_Warehouse_Helper_Catalog_Product_Price_Indexer 
    extends Mage_Core_Helper_Abstract
{
    ...
    {
        $select->joinLeft(
            array($tableAlias => $table), implode(
                ' AND ', array(
    ...
    public function getAggregatedOptionPriceSelect($write, $table)
    {
        ...
        if ($this->getVersionHelper()->isGe1700()) {
            $select->from(
                array($table), array('entity_id', 'customer_group_id', 'website_id', 
    ...
    public function getOptionFinalPriceSelect($write, $table)
    {
        $select = $write->select()
            ->join(
                array('io' => $table), '(i.entity_id = io.entity_id) AND (i.customer_group_id = io.customer_group_id) AND '.
    ...
    public function getDownloadableLinkFinalPriceSelect($write, $table)
    {
        ...
        $select = $write->select()
            ->join(
                array('id' => $table), 'i.entity_id = id.entity_id AND i.customer_group_id = id.customer_group_id'
