...
class Thirdlevel_Pluggto_Model_Line extends Mage_Core_Model_Abstract
{
    ...
    public function playline($force = false,$id=null){
        ...
        try{
            ...
            $queue_quantity = Mage::getStoreConfig('pluggto/configs/queues_quantity');

            if(empty($queue_quantity)){
                $queue_quantity = 100;
            }


            $collection = $this->getCollection();
            $datas = $collection->addFieldToFilter('status',0)->setPageSize($queue_quantity)->setOrder('id', 'DESC');
            ...
            foreach($datas as $data):
                ...
                if(!Mage::getStoreConfig('pluggto/configs/multi_queues') && strtotime($passou) > $allowtime ){
                    break;
    ...
    public function cleanStackForMultiQueue(){
        ...
        $sdate = date('Y-m-d', strtotime("-1 hour")).' 00:00:00';

        $resource = Mage::getSingleton('core/resource');
        $writeConnection = $resource->getConnection('core_write');
        $tableName = $resource->getTableName('pluggto/line');
        $query = "UPDATE " . $tableName . " set status = 0 WHERE status = 3 AND created > '".$sdate."'";
        $writeConnection->query($query);
