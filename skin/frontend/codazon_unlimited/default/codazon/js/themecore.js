...
(function($) {
    ...
    themecore.qtyControl = function() {
        ...
        $('body').on('click','[data-role=change_cart_qty_multiplo]', function (e) {

            var curQtyOriginal = $('#qty_mostrada').data('original');
            var $btn = $(this);
            if ($btn.data('role') != 'change_cart_qty_multiplo') {
                $btn = $btn.parents('[data-role=change_cart_qty_multiplo]').first();
            }
            ...
            curQty += qty;
            curQtyMultiplo = curQty * parseFloat(curQtyOriginal);
            if (curQty <= 0) {
                curQty = 1;
                curQtyMultiplo = curQtyOriginal;
            }
            $qtyInput.val(curQty);
            $('#qty_mostrada').val(parseFloat(curQtyMultiplo).toFixed(2));
            $('.span-multiplo').html(curQty + " embalagem de " + curQtyOriginal + " m² (total de " + parseFloat(curQtyMultiplo).toFixed(2) + " m²)");
