    ...
    $.widget('codazon.productMedia', {
        ...
        _initHTML: function() {
            ...
            $(mainImage).on('load', function() {
                ...
                setTimeout(function() {
                    var config = self._getSliderSettings({});
                    if (self.$moreview.find('[data-role=thumb-item]').length <= config.item) {
                        self.element.find('.lSSlideWrapper').off('touchmove');
                    }
                }, 4000);
                self.$moreview.find('[data-role=thumb-item] a').first().trigger('click');
            });
