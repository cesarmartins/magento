	...
	$.fn.menuLayout = function(options){
		...
		return this.each(function(id,el){
					...
					if(previewUrl != null){
						...
						if(typeof window.Dialog !== 'undefined'){
							...
							var message = {
								...
								options: {
									method: 'post',
									parameters: $previewForm.serialize(true),
									onSuccess: function(originalRequest) {
										Dialog.callFunc = function(){
											previewDialog.info(originalRequest.responseText, previewDialog.parameters)
											$('[data-megamenu]').each(function(){
												var $menu = $(this);
												var data = $menu.data('megamenu');
												$menu.codazonMegaMenu(data);
												$menu.trigger('contentUpdated');
											});
										}
									}
								}
							...
							previewDialog.info(message, {
								...
								title: 'Preview',
								width: Math.min($(window).width()-100,1400),
			...
			if(typeof $.fn.modal !== 'undefined'){
				var modalHtml = '<div id="imgModal" class="modal fade" role="dialog">';
				modalHtml +=		'<div class="modal-dialog modal-lg">';
				modalHtml +=			'<div class="modal-content">';
				modalHtml +=				'<div class="modal-header"><button type="button" class="close" data-dismiss="modal">&times;</button></div>';
				modalHtml +=				'<div class="modal-body"><div class="img-modal"><img src="" /></div></div>'
				modalHtml +=			'</div>';
				modalHtml +=		'</div>';
				modalHtml +=	'</div>';
			}else{
			...
			var cdzmenu = {
				...
				confirm: function(title,content,actions){
					if(typeof conf.confirm == 'function'){
						...
						if(confirm(content)){
							if(typeof actions.confirm !== 'undefined'){
								actions.confirm();
							}
						};
					}
				...
				viewfull: function(obj){
					...
					}else if(typeof window.Dialog !== 'undefined'){
						Dialog.alert($imageModal.html(), {
							draggable:true,
							resizable:true,
							closable:true,
							className:"magento",
